...
def lint(files):
    ...
    if not files:
        files = ("src/tests", "src/twitter_bot_detection")
...
def build_docs(open_docs):
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=twitter_bot_detection"]
    )
...
def _build_jupyter_command(
    ...
    if not all_kernels:
        project_name = "twitter-bot-detection"
...
def convert_notebook(all_flag, overwrite_flag, filepath):
    ...
    *Note*: Make sure your notebooks have unique names!
    ...
    kedro_package_name = "twitter_bot_detection"
